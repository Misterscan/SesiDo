...
Generate a complete webpage with `html()` and write it as an `.html` file:

```sesi
let page = html("
<main>
  <h1>Daily Notes</h1>
  <p>Generated by Sesi at " + str(time()) + "</p>
  <button onclick='alert(\"Saved from Sesi\")'>Test JavaScript</button>
</main>
", {
  "title": "Daily Notes",
  "head": "
...
write_file("daily-notes.html", page)
```
